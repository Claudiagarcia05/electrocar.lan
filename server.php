<?php
    ...
    switch ($request) {
        ...
        case 'get_productos_destacados': getProductosDestacados($conn); break;
        case 'login': 
        ...
        case 'actualizar_stock': 
            if ($method === 'POST') { actualizarStock($conn); } 
            else { http_response_code(405); echo json_encode(["error" => "Método no permitido"]); }
            break;
    ...
    function actualizarStock($conn) {
        ...
        $sql = "UPDATE productos SET disponibilidad = disponibilidad - $cantidad WHERE id = $producto_id";
